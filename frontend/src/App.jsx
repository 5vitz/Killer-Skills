        ...
        {activeView === "servicos" && (
          <div className="relative w-full h-full flex justify-center items-center">
            ...
            <div className="fixed left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 w-[340px] h-[550px] bg-brand-card border-2 rounded-[45px] p-4 flex flex-col justify-between items-center shadow-2xl transition-all duration-300 z-30" style={{ borderColor: currentPersona.color }}>
              ...
              {/* Header Acima do Smartphone (Absoluto para não empurrar o player!) */}
              <div className="absolute bottom-[calc(100%+16px)] left-1/2 -translate-x-1/2 flex items-center gap-4 whitespace-nowrap z-40">
                <button 
                  onClick={() => scrollPersona("up")}
                  className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex justify-center items-center hover:bg-white/10 active:scale-90 duration-100"
                >
                  <ChevronUp className="w-4 h-4" />
                </button>
                <div className="text-[10px] font-bold tracking-widest text-white/30 uppercase">
                  Rodante Reels ({String(selectedPersonaIdx + 1).padStart(2, '0')} / {activePersonasList.length})
                </div>
                <button 
          ...
          <div className="relative w-full h-full flex justify-center items-center">
            ...
            <div className="fixed left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 w-[340px] h-[550px] bg-brand-card border-2 rounded-[45px] p-4 flex flex-col justify-between items-center shadow-2xl transition-all duration-300 z-30" style={{ borderColor: currentPersona.color }}>
              ...
              <div className="absolute bottom-[calc(100%+16px)] left-1/2 -translate-x-1/2 flex items-center gap-4 whitespace-nowrap z-40">
                ...
                <div className="text-[10px] font-bold tracking-widest text-white/30 uppercase">
                  Rodante Reels ({String(selectedPersonaIdx + 1).padStart(2, '0')} / {personas.length || 24})
                </div>
                <button 
                  onClick={() => scrollPersona("down")}
                  className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex justify-center items-center hover:bg-white/10 active:scale-90 duration-100"
