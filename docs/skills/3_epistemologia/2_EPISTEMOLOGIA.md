...
| **Tela 1D** *(Diagnóstico)* | **Painel MYSELF / OURSELVES** (Diagnóstico no topo, botão **`✎ EDITAR PERSONA`** na base e botão sutil **`VOLTAR`** no rodapé inferior apontando para a Tela 1C). | Título `ESPELHO DA ALMA` + **Grid dos sliders travados** em modo de visualização. | **Chassi CRIAR PROMPT** (Título `CRIAR PROMPT` + Texto coloquial de ajuda + As **4 caixas de refino** com trava de segurança + Botão **`✓ CONFIRMAR PERSONA`** na base) + Controle de Áudio na base. |
| **Tela 2** *(Cockpit)* | **Limpa** (Com botão sutil **`VOLTAR`** no rodapé inferior apontando para a Tela 1D). | Título `DEFINIÇÕES DO POST` + **Configurações Físicas** (Formato do Post, Quantidade, pílulas de tags táteis) + Botão dourado **`EMITIR ORDEM DE SERVIÇO`**. | **Limpa** (Controle de Áudio na base). |
| **Tela 3** *(KS Studio)* | **Limpa** (Com botão **`< VOLTAR AO PAINEL`** na base redirecionando para a Tela 1D). | **Simulador de Feed do Instagram (1:1)** (Miniatura interativa do post reels/carrossel). | **Cockpit Vazio** (Card reservado para a forja estética) + Controle de Áudio. |
